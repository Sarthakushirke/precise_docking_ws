...
class LaserToPointCloud(Node):
    ...
    def listener_callback(self, msg):
        ...
        if self.prev_scan is not None and self.is_data_changed():
            ...
            # Compare the transformed Q with P (previous scan)
            # print("Difference between P and transformed Q:", np.linalg.norm(P - Q_transformed, axis=1))

            # Plot both previous and current scans
            # self.plot_previous_and_current()
    ...
    def plot_previous_and_current(self):
        """Plots two separate figures for previous and current scans and automatically switches between them."""

        # Plot the previous scan
        if self.prev_scan is not None:
            plt.figure(figsize=(8, 6))
            plt.scatter(self.prev_scan[:, 0], self.prev_scan[:, 1], c='red', marker='o')
            plt.title('Previous Scan')
            plt.xlabel('X Coordinate (meters)')
            plt.ylabel('Y Coordinate (meters)')
            plt.grid(True)

            # Display the plot for a short time (2 seconds)
            plt.pause(2)
            plt.close()  # Close the figure after pausing

        # Plot the current scan
        if self.current_scan is not None:
            plt.figure(figsize=(8, 6))
            plt.scatter(self.current_scan[:, 0], self.current_scan[:, 1], c='blue', marker='o')
