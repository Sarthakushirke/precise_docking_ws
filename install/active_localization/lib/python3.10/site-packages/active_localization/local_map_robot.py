...
class OccupancyGridUpdater(Node):
    ...
    def map_callback(self, msg):
        self.map_info = msg.info
        raw_map_data = np.array(msg.data, dtype=np.int8).reshape((self.map_info.height, self.map_info.width))

        # Exclude occupied cells (100), set them to -1 (unknown)
        self.map_data = np.where(raw_map_data == 100, -1, raw_map_data)

        self.resolution = self.map_info.resolution
        self.width = self.map_info.width
        self.height = self.map_info.height

        self.get_logger().info('Map callback processed. Occupied cells are ignored.')
    ...
    def lidar_callback(self, msg):
        ...
        FIELD_OF_VIEW = 270.0  # Field of view in degrees
        FIELD_OF_VIEW_RAD = np.deg2rad(FIELD_OF_VIEW)  # Convert to radians

        # Get angle range
        angles = np.linspace(angle_min, msg.angle_max, len(ranges))

        # Define the allowable angle range centered around the middle
        angle_mid = (msg.angle_max + msg.angle_min) / 2.0
        ...
        ranges = np.where(np.isinf(ranges), MAX_LIDAR_RANGE, ranges)

        # Apply range and angle filtering
        valid_indices = (ranges <= MAX_LIDAR_RANGE) & (angles >= angle_min_allowed) & (angles <= angle_max_allowed)

        # Keep only valid ranges and angles
