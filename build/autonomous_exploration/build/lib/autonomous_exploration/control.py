...
def findClosestGroup(matrix,groups, current,resolution,originX,originY):
    ...
    else: #If the groups are closer than target_error*2 distance, it chooses a random point as target. This allows the robot to escape from some situations.
        index = random.randint(0,len(groups)-1)
        target = groups[index][1]
        target = target[random.randint(0,len(target)-1)]
        path = astar(matrix, current, target)
...
def exploration(data,width,height,resolution,column,row,originX,originY):
        ...
        data = costmap(data,width,height,resolution) #(Expand the barriers) Engelleri genislet
        data[row][column] = 0 #Robot Current Location (This will come from AMCL)
        ...
        if len(groups) == 0: #If there is no group, the exploration is complete
            ...
            data[data < 0] = 1 #-0.05 is an unknown place. Mark it as unvisitable. 0 = accessible, 1 = unvisitable.
...
class navigationControl(Node):
    def __init__(self):
        ...
        self.kesif = True  #kesif means discovery or exploration in turkish
        threading.Thread(target=self.exp).start() #Kesif fonksiyonunu thread olarak calistirir.
        
    def exp(self):
        twist = Twist()
        while True: # Wait until sensor data comes in.
            ...
            if self.kesif == True:
                ...
                t = t - 0.2 #0.2 seconds is subtracted from the calculated time according to the formula x = v * t. After t time, the exploration function is run.
